...
if [ "$(echo $SSH_CLIENT | awk '{print $1}')" = "10.11.55.99" ] || [ "${HOST}" = "zhangcl-neon" ]; then
    ...
    ZSH_THEME_GIT_PROMPT_UNTRACKED="%{$FG[160]%}🛠️ %{$reset_color%}"
    ZSH_THEME_GIT_PROMPT_UNMERGED="%{$FG[039]%}⛏️ %{$reset_color%}"

    icon_hourglass="⏳%{$reset_color%}"
    icon_prompts_ko="⛔%{$reset_color%}"
    icon_date="📆 %{$reset_color%}"
    icon_ip="%{$FG[039]%}🌐 %{$reset_color%}"
else
    local DIRTY_STRING=""
    ZSH_THEME_GIT_PROMPT_PREFIX="( "
    ZSH_THEME_GIT_PROMPT_SUFFIX=")"
...
bureau_git_prompt () {
  ...
  #if [ "${_pwd}" =~ "xxx" ]; then
  if [ "${ZSH_THEME_GIT_PROMPT}" = "no" ]; then
